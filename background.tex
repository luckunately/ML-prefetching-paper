...
One of the recent state-of-art prefetcher, Leap~\cite{leap}, improved traditional prefetching using majority trend detection to identify stride patterns; this makes Leap resilient to short-term irregularities in the memory access stream.
Leap improved performance for many applications but cannot prefetch irregular accesses. 
Multiple researchers have tried to improve Leap's performance by addressing its shortcomings~\cite{canvas, dilos, memliner} in \begin{itemize}
...
\textbf{WHAT ARE THE SHORTCOMINGS OF TRADITIONAL PREFETCHING?}

One stunning outcome of our initial experiments was that the \texttt{lbm} benchmark from SPEC2017~\cite{SPEC2017} revealed the huge flaw in LEAP~\cite{leap}. \textbf{ADD THE LINK TO THE GRAPH LATER} LEAP prefetches pages based on the majority trend of the memory access stream. However, the \texttt{lbm} benchmark has an simple alternating access pattern that LEAP cannot predict since none of the strides are in the majority. This resulted in LEAP prefetching pages that were never accessed, leading to a large number of page faults. 
...
Studies~\cite{Aggressive} has shown the need for more aggressive prefetching to reduce the number of page faults while sacrificing some portability and memory bandwidth. Thus, we propose a machine learning based prefetcher that can improve the prefetching accuracy and reduce the number of page faults.
...
\textbf{WHAT IS THE GAP OUR WORK IS TRYING TO FILL? WHAT IS THE PREVIOUS WORK MISSING?}
...
\begin{enumerate}
    \item Compute the input gate $i_t$, forget gate $f_t$, and output gate $o_t$ in parallel.\begin{align*}
...
\textbf{MAYBE MORE CITATIONS ON PREFETCHING USING LSTM}
LSTM has shown promising results in cache prefetching~\cite{LMAP,Twilight} due to its ability to learn long-term dependencies. However, LSTM has a large number of parameters and is computationally expensive to train and run. 
